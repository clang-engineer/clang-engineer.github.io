...
> **언어 공통 개념**: "타입을 매개변수로 받는" 이 아이디어는 C++만의 것이 아니라 **파라미터 다형성**이라는 언어 무관 개념입니다. 단형화 vs 타입 소거, 제약(constraints)이 언어마다 갈리는 지점은 → [제네릭 — 파라미터 다형성](https://github.com/clang-engineer/clang-engineer.github.io/blob/main/_archive/정보관리기술사/05-소프트웨어-공학/_보충학습/프로그래밍-언어-의미론.md). 이 글은 그 개념의 **C++ 구현(템플릿)**에 집중합니다.
