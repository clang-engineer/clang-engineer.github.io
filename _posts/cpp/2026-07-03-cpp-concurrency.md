...
- **코루틴(C++20, `co_await`)** — 스레드를 늘리지 않고 수만 개의 비동기 작업을 한 스레드로 돌리는 경량 동시성. 로드맵 [필수] 밖이지만, 스레드와 무엇이 다른지·C++20 기계(`promise_type`·`coroutine_handle`)·이벤트 루프는 언어 공통 개념으로 → [코루틴이란 무엇인가](../concept/2026-07-12-coroutine.md).
