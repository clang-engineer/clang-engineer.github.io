...
description : "C++ 입문부터 모던 C++까지의 학습 순서를 초급·중급·고급으로 묶고, 각 주제를 필수·나중·선택으로 표시한 로드맵. 순서의 근거, 선행 지식, 자주 막히는 지점을 함께 정리한다."
...
**[필수]만 세로로 따라가면 하나의 완결된 경로**입니다. 아래는 각 단계를 왜 그 순서·우선순위로 두는지 풀어 쓴 것입니다.
...
- **[필수] 빌드 시스템(Make & CMake)** — 여러 파일로 나뉜 프로젝트 빌드. 여기에 [cpplint·clang-format](/posts/cpp/2024-10-22-cpplint-clangformat/) 같은 도구도 붙습니다.
- **[선택] 실무 스타일** — 표준 라이브러리의 디자인 패턴, 대규모 팀의 C++ 사용 관례

## 이 로드맵을 쓰는 법

- **[필수]만 따라가도 하나의 완결된 경로**가 됩니다. [나중]·[선택]은 실무에서 필요해질 때 돌아오세요.
