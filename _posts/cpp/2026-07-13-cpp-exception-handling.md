...
> **아, `std::move`를 배울 때 "이동 생성자는 noexcept로"라던 게 이거였다.** 이동을 `noexcept`로 표시하지 않으면 `vector`가 이동을 포기하고 복사로 후퇴해, 애써 만든 이동 시맨틱이 무력화됩니다. → [이동 시맨틱과 스마트 포인터](/posts/cpp/2026-07-03-cpp-move-and-smart-pointers/)
