...
> 관련: [AI 로드맵](./2026-07-03-ai-roadmap.md)의 **실전 워크플로** 갈래 · [Claude Code 실전 워크플로](./2026-07-03-claude-code-workflow.md)에 이어지는 확장 편.

플랜·서브에이전트·헤드리스로 [Claude Code를 굴리는 법](./2026-07-03-claude-code-workflow.md)에 익숙해지면, 다음은 **도구 자체를 확장**하는 단계다. 확장의 한 축이 **hooks** — 도구 실행 흐름에 셸을 끼워 넣는 장치다. (다른 축인 **커스텀 슬래시 명령**은 [슬래시 명령어 사전](./2025-10-24-claude-code-slash-commands.md#커스텀-명령어-만들기)에서 다룬다.)
...
- **커스텀 슬래시 명령** — 반복 프롬프트를 `/이름` 하나로. 저장 위치·`$ARGUMENTS`·frontmatter(`allowed-tools`·`model`)·`!명령`/`@파일` 전처리까지는 [슬래시 명령어 사전 › 커스텀 명령어 만들기](./2025-10-24-claude-code-slash-commands.md#커스텀-명령어-만들기)에 정리해 두었다.
...
역할로 나누면 — **hooks는 "무엇을 자동으로 실행할까"**(포맷·차단·알림), **커스텀 명령/스킬은 "무엇을 시킬까"**(프롬프트 재사용). 셋을 합치면 Claude Code를 팀·프로젝트에 맞게 길들이는 도구가 갖춰진다. 전체 학습 경로는 [AI 로드맵](./2026-07-03-ai-roadmap.md)에서.
