...
- **커스텀 슬래시 명령** — 반복 프롬프트를 `/이름` 하나로 부른다. 저장 위치·`$ARGUMENTS`·frontmatter·`!명령`/`@파일` 전처리는 [슬래시 명령어 사전](./2025-10-24-claude-code-slash-commands.md#커스텀-명령--반복-프롬프트를-이름으로-만든다)에 정리해 두었다.
