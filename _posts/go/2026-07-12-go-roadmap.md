...
> **검토 기준:** Go 1.26. 언어 specification과 표준 도구 동작은 이 기준으로 확인했습니다.
...
- **[필수] map** — `unordered_map` 대응. 순회 순서는 **지정되지 않으므로** 어떤 순서도 전제하지 않습니다.
- **[필수] string과 rune** — string은 임의 바이트열입니다. UTF-8 텍스트를 다룰 때 `byte`, `rune`(코드포인트), 화면상 grapheme cluster를 구분합니다.
...
- **[필수] race detector(`-race`)** — `go test -race`, `go run -race`로 실행 중 데이터 레이스를 탐지. 동시성 코드를 배포하기 전 반드시 거치는 검증 단계입니다. goroutine 누수는 별도로 종료 경로와 profiling으로 확인합니다.

**자주 막히는 지점:** 아무도 받지 않는 채널에 보내거나 더는 값이 오지 않는데 받으려고 기다려 생기는 **deadlock**. 채널은 수신자에게 "더는 값이 없다"고 알려야 할 때만 닫으며, 모든 채널을 반드시 닫아야 하는 것은 아닙니다. 눈에 안 보이는 데이터 레이스는 `-race`로 검사합니다.
...
| `std::unordered_map` | `map` | 순회 순서 미지정 |
