...
이 영역의 사실상 표준은 [Rectangle.app](https://rectangleapp.com/)이다. 무료·오픈소스이고, 설치 후 `Ctrl+Opt+←/→`로 반분할, `Ctrl+Opt+↩`로 최대화처럼 기본 단축키만으로 바로 쓸 수 있어 macOS 창 분할 앱 중 가장 널리 쓰인다. **막 시작한다면 [Rectangle.app부터 깔아 보는 것](./2026-07-03-rectangle-app-basics.md)이 가장 쉬운 길이다.**
