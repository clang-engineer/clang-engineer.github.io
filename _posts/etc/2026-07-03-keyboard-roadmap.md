...
description : "세벌식 390·HHKB로 손버릇의 배경을 잡고, 펌웨어 지형도·용어로 도구 지도를 그린 뒤, 그 손버릇을 ZMK 40% 커스텀 키맵으로 옮기는 여정이 이 로드맵의 척추. 같은 펌웨어 레이어의 다른 경로(QMK·VIA·VIAL)는 곁길로 가볍게 얹고, 펌웨어를 아예 못 건드릴 때의 OS 레이어(Karabiner)만 결이 다른 축으로 분리한다."
...
실전에서 "키를 바꾸는" 방법은 **어느 레이어에서 바꾸느냐**로 갈린다. ZMK·QMK·VIA·VIAL은 모두 **보드 펌웨어를 고치는 같은 레이어**의 다른 경로다 — 서로 결이 다른 게 아니라 같은 목적지로 가는 다른 길이라, 척추(ZMK) 옆에 곁길로 둔다. 반면 리플래시가 아예 안 되는 보드를 OS가 가로채 덮는 **Karabiner는 펌웨어 밖 OS 레이어** — 이것만 결이 다른 축이라 아래로 분리했다.

## 한눈에 보기

배경 → 도구 지도 → ZMK 실전이 척추다. QMK·GUI는 같은 펌웨어 레이어의 곁길, Karabiner만 다른 축이다.
...
| 다른 경로 | 같은 펌웨어 레이어 — QMK·VIA·VIAL | 곁길 |
| 다른 축 | OS 레이어 — 펌웨어 못 건드릴 때 Karabiner | 다른 축 |
...
## 다른 경로 — 같은 펌웨어 레이어 (QMK·VIA·VIAL)

이 블로그가 깊게 다루는 건 저자의 길인 ZMK지만, 같은 펌웨어 레이어엔 다른 경로도 있다. 유선 보드면 QMK, 컴파일이 번거로우면 GUI(VIA·VIAL). ZMK와 "다른 축"이 아니라 같은 목적지로 가는 다른 길이라, 필요한 사람이 출발점을 잡도록 곁에 얹어 둔다. 설계 원칙(레이어 분리·hold-tap)은 위 ZMK 설계기에서 그대로 가져오고, 표기·빌드 흐름만 바꾸면 된다.
...
## 다른 축 — OS 레이어 (Karabiner)

리플래시가 아예 불가능한 키보드(맥북 내장, 기성품)는 펌웨어를 못 건드린다. 이때는 OS 레벨에서 덮는 게 유일한 길 — 위 펌웨어 레이어와 **완전히 분리된 축**이라 따로 뺐다.
...
- **유선 보드거나 컴파일이 번거로우면** → 다른 경로의 QMK, 혹은 VIA·VIAL. 설계 원칙은 ZMK 설계기에서 가져온다.
- **맥북 내장·리플래시 불가한 키보드까지 리맵하려면** → 다른 축의 Karabiner로 OS 레벨에서 덮는다.
