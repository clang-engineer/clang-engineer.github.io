...
updated     : 2026-09-13 19:20:00 +0900
categories  : [terminal]
...
앞에서는 애플리케이션이 표준 출력으로 ANSI/VT 계열 제어 시퀀스를 보내고, 터미널 에뮬레이터가 이를 해석해 화면을 바꾼다는 구조를 봤다.
...
`terminfo`는 그 앞에서 **현재 터미널에 어떤 시퀀스를 써야 하는지 선택하는 데 도움을 주는 호환성 정보**라고 보면 된다.
...
하지만 현실의 터미널 생태계에서는 모든 터미널이 모든 기능과 확장을 완전히 동일하게 지원하는 것은 아니다.
...
그래서 호환성을 중요하게 생각하는 프로그램은 **현재 상대하는 터미널이 무엇을 지원하는지 확인한 뒤 적절한 시퀀스를 선택**할 수 있다.
...
> **ANSI/VT는 실제 제어 언어이고, terminfo는 필요할 때 현재 터미널에 맞는 사용법을 선택하도록 돕는 정보 계층이다.**
...
중요한 것은 Vim이 매번 다음처럼 제품명을 직접 분기하는 구조가 아니라는 점이다.
...
공통 터미널 문법 위에서 현재 터미널이 어떤 기능을 지원하고, 그 기능을 어떤 시퀀스로 호출해야 하는지를 기술하는 호환성 정보에 가깝다.
