...
또 터미널 인터페이스마다 capability 차이가 있으므로 `TERM`과 `terminfo`를 이용해 적절한 제어 시퀀스를 선택할 수 있었다.
...
```text
Screen
= 터미널 에뮬레이터의 표시 영역 전체를 하나의 논리적 화면으로 다룸
...
refresh()  → 터미널 에뮬레이터에 반영
getch()    → 입력 받기
endwin()   → 터미널 상태 복구
```

## Screen — 전체 표시 영역을 논리적 화면으로 본다

curses는 터미널 에뮬레이터의 표시 영역 전체를 하나의 **논리적 화면(Screen)**처럼 다룬다.
...
## refresh — 원하는 화면 상태를 실제 출력으로 반영한다

`mvaddstr()`로 문자열을 지정했는데 왜 `refresh()`가 또 필요할까?

핵심은 curses가 **애플리케이션이 원하는 화면 상태**와 **curses가 추적하는 현재 화면 상태** 사이에 중간 모델을 두기 때문이다.
...
즉 curses는 단순 출력 함수 모음이 아니라 **화면 상태를 관리하고 필요한 제어 시퀀스를 터미널 에뮬레이터에 출력하는 계층**이다.

이게 `refresh()`의 핵심 의미다.

## 입력도 한 단계 올려준다

방향키 같은 입력은 여러 바이트의 escape sequence일 수 있다.

```text
위쪽 방향키
  ↓
터미널 에뮬레이터가 입력 시퀀스 전송
  ↓
ESC [ A
  ↓
curses 입력 처리
...
ncurses는 `wnoutrefresh()`와 `doupdate()` 같은 API로 여러 Window의 변경을 모은 뒤 실제 출력 갱신을 한 번에 수행할 수 있다.
