...
description : "현대 TUI 프레임워크를 관통하는 입력 이벤트 → 상태 갱신 → 레이아웃 → 셀 버퍼 → 차이 렌더링 구조를 공통 질문과 처리 흐름으로 정리한다."
...
하지만 실제 대화형 TUI 애플리케이션을 만들려면 한 단계 더 높은 구조가 필요하다. 사용자가 키를 누르고, 상태가 바뀌고, 레이아웃을 계산하고, 화면을 다시 그리는 전체 실행 흐름이다.

프레임워크마다 API 이름은 달라도 먼저 다음 질문으로 보면 공통 구조가 잡힌다.

```text
1. 무엇이 앱의 변화를 시작시키는가?
   → 키, 마우스, 크기 변경, 타이머, I/O 결과 같은 Event
...
4. 그 화면을 무엇으로 표현할 것인가?
   → Terminal Cell Buffer

5. 실제 Terminal에는 무엇을 출력할 것인가?
   → 이전 화면과 비교한 변경분
```
...
이 구조를 이해하면 Bubble Tea, Ratatui, Textual, OpenTUI가 서로 완전히 다른 세계가 아니라 **같은 TUI 문제에서 서로 다른 범위를 대신해주는 프레임워크**라는 점이 보인다.

## 1. 입력 — 변화의 시작은 Event다

가장 아래의 입력 경로는 앞에서 본 그대로다.
...
## 4. 레이아웃 — State를 Terminal 좌표로 배치한다
...
TUI Renderer의 대표적인 내부 표현은 **2차원 셀 버퍼(Cell Buffer)**다.
...
즉 TUI Renderer가 다루는 화면은 픽셀 Framebuffer보다 **터미널의 문자 셀 격자**에 가깝다.
...
그렇다고 실제 Terminal에 모든 셀을 다시 출력하는 것은 아니다. 실제 출력량은 뒤의 Diff 단계에서 줄일 수 있다.
...
> **어떤 방식으로 UI를 기술하더라도 결국 다음에 보여줄 터미널 셀 상태를 만들고 실제 출력 단계로 내려간다.**

## 7. 차이 렌더링 — 실제 Terminal에는 변경분만 보낸다
...
화면 전체를 지우고 다시 출력할 수도 있다. 하지만 Renderer가 두 Buffer를 비교하면 바뀐 셀만 찾을 수 있다.
...
Ratatui는 이전/현재 Buffer 차이를 계산해 필요한 변경을 Terminal에 쓰고, OpenTUI도 두 Render Buffer의 차이를 이용해 변경된 셀을 출력한다.

curses의 가상 화면과 실제 화면 모델도 같은 문제를 풀기 위한 앞선 형태로 볼 수 있다.

여기서 마지막 질문이 나온다.

> **다음 화면 전체 중 실제 Terminal에 꼭 보내야 하는 변경분은 무엇인가?**

## 8. 이중 버퍼 — 다음 화면을 화면 밖에서 완성한다
...
대신 다음 Frame을 별도의 Buffer에 완성한 뒤 현재 Buffer와 비교할 수 있다.
...
이 구조를 사용하면 화면 갱신을 하나의 Frame 단위로 다룰 수 있다.
...
> **State가 바뀐 뒤 언제 다음 화면을 계산하고 Terminal에 반영할 것인가?**
