...
updated     : 2026-09-05 13:10:00 +0900
categories  : [terminal]
...
이전 글에서 현대 터미널 에뮬레이터가 PTY를 만들고, shell이나 TUI 애플리케이션은 그 PTY의 slave 쪽을 실제 터미널처럼 사용한다는 구조를 봤다.

그런데 여기서 바로 다음 의문이 생긴다.

- 평범한 shell에서는 왜 `abc`를 입력해도 Enter를 누르기 전까지 프로그램이 못 받는가?
...
애플리케이션이 키보드에서 온 바이트를 그대로 받는다고 생각하기 쉽지만, 기본 상태에서는 그렇지 않다. 커널의 terminal driver가 입력을 중간에서 가공한다.
...
기본 shell 입력은 대개 **canonical mode**다.

```text
사용자: h e l l o Enter
                 │
terminal driver가 한 줄을 모음
                 │
                 ▼
Application read()
        "hello\n"
...
terminal driver가 내부 버퍼에서 한 줄을 편집한 뒤 Enter 같은 line delimiter가 들어오면 그제야 프로그램에 전달한다.

그래서 shell에서 입력 중 Backspace를 누르면 애플리케이션이 직접 이전 글자를 지우는 것이 아니라 terminal driver가 canonical line editing을 처리할 수도 있다.
...
화면에 `hello`가 보이니 shell이 타이핑한 글자를 화면에 다시 출력한다고 생각하기 쉽다.

하지만 기본 상태에서는 `ECHO`가 켜져 있어서 **terminal driver가 입력 문자를 다시 출력**한다.

```text
keyboard input
      ↓
terminal driver
   ├─ application 쪽 input buffer
   └─ 화면 쪽으로 echo
```
...
하지만 shell에서 실행 중인 프로그램에 `Ctrl-C`를 누르면 일반적으로 프로그램은 문자 `0x03`을 읽지 않는다. 대신 `SIGINT`를 받는다.
...
중요한 점은 signal이 **terminal emulator가 직접 프로세스에 보내는 것이 아니라 terminal driver가 foreground process group에 전달**한다는 것이다.

그래서 job control과 controlling terminal 개념이 뒤에서 연결된다.

## Noncanonical mode — 한 줄이라는 개념을 없앤다

`ICANON`을 끄면 terminal driver는 더 이상 Enter까지 한 줄을 모으지 않는다.
...
```text
ICANON  → line buffering/line editing 끔
ECHO    → 입력 echo 끔
...
```text
Canonical mode
Keyboard → terminal driver가 해석/편집 → Application

Raw-ish mode
Keyboard → 최소 가공 → Application이 직접 해석
```

그래서 TUI 앱은 방향키 escape sequence, `Ctrl-C`, `j`, `k` 등을 자신의 event loop에서 구분할 수 있다.
...
실험 중 terminal을 완전히 망가뜨렸다면 새 terminal tab을 열어 복구하는 것도 방법이다.
...
| VMIN | VTIME | 의미 |
|---:|---:|---|
| 1 | 0 | 최소 1 byte가 올 때까지 block |
| 0 | 0 | 데이터 없으면 즉시 반환, polling |
| 0 | >0 | 일정 시간 기다렸다 없으면 timeout |
| >0 | >0 | 최소 byte 수 + inter-byte timeout 조합 |

TUI event loop는 이 설정 또는 `poll`/`select`/`epoll`/`kqueue` 같은 I/O multiplexing을 조합해 입력을 기다린다.

여기서부터 터미널 입력은 UI 프레임워크의 **event source**가 된다.

## 방향키는 한 글자가 아니다

raw mode를 켰다고 해서 모든 키가 한 byte라는 뜻은 아니다.

예를 들어 터미널의 방향키는 보통 여러 byte로 이루어진 escape sequence를 보낸다.

개념적으로 Up Arrow는 다음과 비슷하다.
...
그래서 터미널 프로그램은 종료·signal 처리 시 terminal state restoration을 중요하게 다룬다.

## shell의 line editing은 또 어디에 있나

여기서 한 가지 헷갈릴 수 있다.

canonical mode 자체에도 기본적인 line editing이 있지만, 현대 shell의 화려한 편집 기능은 그것보다 위에서 구현된다.
...
같은 기능은 shell/readline/zle 같은 애플리케이션 계층이 terminal을 적절히 제어하면서 구현한다.

즉:

```text
terminal driver의 canonical editing
        vs
shell이 직접 만드는 interactive line editor
```

는 같은 것이 아니다.

현대 interactive shell은 더 풍부한 기능을 위해 terminal을 직접 제어하는 부분이 많다.
...
TUI 프로그램은 픽셀을 그리는 대신 stdout으로 특별한 byte sequence를 내보낸다.
...
즉 다음 주제는 **ANSI/VT Escape Sequence — stdout으로 커서를 움직이고 화면을 그리는 방법**이다.
