...
stty
= 현재 터미널의 termios 설정을 조회하거나 바꾸는 사용자 명령어
```

즉 `termios`라는 별도 프로그램이 입력을 받아 다시 전달하는 것이 아니다. **PTY/TTY의 터미널 드라이버가 termios 설정에 따라 입력을 가공하고, 셸이나 애플리케이션은 그 결과를 `read()`로 받는다.**

감각적으로는 키 입력을 터미널 규칙에 맞게 한 번 "번역"해서 애플리케이션에 넘긴다고 볼 수 있다. 다만 모든 입력이 문자로 다시 전달되는 것은 아니다. 예를 들어 `ISIG`가 켜져 있으면 `Ctrl-C` 같은 특수 입력은 문자 대신 Unix Signal로 바뀌어 foreground process group에 전달될 수 있다.
...
`stty`는 **현재 터미널에 적용된 termios 설정을 조회하거나 변경하는 명령어**다.

가장 간단한 관찰 방법은 다음과 같다.
