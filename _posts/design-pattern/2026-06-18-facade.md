...
> **난이도** 중급 · **선행** [Adapter](/posts/design-pattern/2025-04-12-adapter/)
...
- **Spring `JdbcTemplate`**: 복잡한 JDBC 절차를 단순화한다는 Facade 성격이 있지만, 핵심 구현은 Template Callback과 자원 관리가 결합된 abstraction이다.
- **HikariCP `DataSource` 진입점**: 내부 pool을 단순한 API 뒤에 숨긴다는 Facade 관점으로 볼 수 있지만, `getConnection()` 한 메서드만으로 패턴 구조가 증명되는 것은 아니다.
