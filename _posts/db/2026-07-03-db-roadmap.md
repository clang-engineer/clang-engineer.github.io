...
description : "트랜잭션·조인·인덱스·옵티마이저·MVCC 원리 → 정규화·파티셔닝·쿼리 튜닝·캐싱 같은 성능 설계 → PostgreSQL 운영(백업·PITR·복제·모니터링·업그레이드) → CAP·샤딩·합의·NoSQL 분산 이론 → MyBatis·JPA·Liquibase·Kibana 도구 연동 → 장애 트러블슈팅까지, 이 블로그의 DB 글 28편을 단계별로 큐레이션."
...
- **MyBatis·JPA·Liquibase·Kibana를 붙이는 중이라면** 도구·연동.
