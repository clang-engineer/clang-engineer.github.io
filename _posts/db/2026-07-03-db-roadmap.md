...
description : "트랜잭션·조인·인덱스·옵티마이저·MVCC 원리 → 정규화·파티셔닝·쿼리 튜닝·캐싱 같은 성능 설계 → PostgreSQL 운영(백업·PITR·복제·모니터링·업그레이드) → CAP·샤딩·합의·NoSQL 분산 이론까지가 학습 척추. 애플리케이션 연동(MyBatis·JPA·Liquibase·Kibana)과 장애 트러블슈팅은 결이 다른 축의 부록으로 분리. 이 블로그의 DB 글 28편을 단계별로 큐레이션."
...
DB는 "쿼리 짜는 법"에서 멈추기 쉽다. 그런데 한 겹만 걷어내면 — 트랜잭션이 왜 이상 현상을 막는지, 옵티마이저가 조인 전략을 어떻게 고르는지, 커넥션 수를 무엇으로 정하는지, 운영 중 데이터를 어떻게 옮기고 올리는지 — 서비스의 안정성과 성능이 갈린다. 이 블로그의 DB 글 28편을 그 층위대로 묶었다. 본인 위치에서 가까운 단계부터 진입하면 된다.

**원리 → 성능 → 운영 → 이론**이 실력이 쌓이는 학습 척추다. 애플리케이션과 DB를 잇는 **도구·연동**, 그리고 특정 장애 **트러블슈팅**은 결이 다른 축이라 아래 **부록**으로 분리했다 — 학습 흐름과 별개로, 필요할 때 찾아 들어오면 된다.

**전제와 범위** — 이 로드맵은 `SELECT`·`JOIN`·`GROUP BY` 같은 기본 SQL은 쓸 줄 안다고 보고 그 위층부터 시작한다. 윈도우 함수·CTE·재귀 쿼리 같은 **SQL 작성 심화**는 여기서 다루지 않는다 — 쿼리를 *어떻게 실행·튜닝하나*는 성능 단계에서 짚지만, *어떻게 작성하나*의 심화는 결이 다른 축이라 이 척추 밖에 둔다.

## 한눈에 보기

원리 → 성능 → 운영 → 이론 4단계가 DB 실력을 쌓는 학습 척추다. 부록은 단계가 아니라 필요할 때 직행하는 별도 트랙이다.

| 구역 | 무엇을 다루나 | 성격 |
|---|---|---|
| 원리 | 트랜잭션·조인·인덱스·옵티마이저·MVCC | 척추 |
| 성능 | 정규화·커넥션·파티셔닝·쿼리 튜닝·캐싱 | 척추 |
| 운영 | PostgreSQL 백업·PITR·복제·모니터링·업그레이드 | 척추 |
| 이론 | CAP·샤딩·합의·NoSQL 분산 이론 | 척추 |
| 부록 A | 도구·연동 — MyBatis·JPA·Liquibase·Kibana | 다른 축 |
| 부록 B | 연결·성능 장애 트러블슈팅 | 별도 트랙 |
...
원리를 잡았으면, 성능이 실제로 갈리는 지점들 — 스키마를 어떻게 정규화·설계하고, 커넥션·조인·파티셔닝·쿼리를 어떻게 다루고, 앞단에 캐시를 어떻게 두는지.
...
여기서부터는 DB 엔진을 이해·운영하는 학습 척추가 아니라, 애플리케이션과 DB를 잇는 **다른 축**이다 — 매핑(MyBatis·JPA), 마이그레이션(Liquibase), 시각화(Kibana). 학습 흐름과 분리해, 해당 도구를 붙일 때 찾아 들어오면 된다.
...
DB 엔진·운영을 배우는 학습 척추와 분리해 둔다. 특정 에러·성능 장애로 막혔을 때 직행하는 글들.
...
- **성능이 어디서 갈리는지 감을 잡고 싶다면** 성능 단계 — 스키마 설계·커넥션 사이징·조인 회피·파티셔닝·쿼리 튜닝·캐싱.
- **운영 중 백업·복제·모니터링·업그레이드가 닥쳤다면** PostgreSQL 운영 글로 직행.
- **분산 스토어 선택·확장의 밑그림이 필요하다면** CAP 정리·분산 확장·NoSQL 글.
