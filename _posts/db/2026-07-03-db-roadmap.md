...
다섯 구역은 각각 질문 하나에 답한다. 이 질문이 곧 "이 글이 왜 여기 있나"의 답이다. 부록은 이 질문들 밖의 별도 트랙이다.
...
여기서부터는 위 다섯 질문 어디에도 안 들어가는 **다른 축**이다 — 애플리케이션과 DB를 잇는 매핑(MyBatis·JPA), 마이그레이션(Liquibase), 시각화(Kibana). 학습 흐름과 분리해, 해당 도구를 붙일 때 찾아 들어오면 된다.
