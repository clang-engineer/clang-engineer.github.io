---
title       : "Neovim 로드맵 — 잘 쓰는 트랙과 만드는 트랙, 두 갈래로 읽는 글 지도"
description : "Neovim 글을 하나의 사다리가 아니라 두 트랙으로 갈랐다. 사용자 트랙(어떻게 잘 쓰나 — 편집 기본기 → LazyVim 구조 이해)과 개발자 트랙(어떻게 만드나 — Lua → 런타임 API → 플러그인 구조·발행·테스트). 둘은 위아래가 아니라 평행 갈래라, 사용자 트랙만 타도 완결된다. 공통 입문에서 갈라지고, 비교·생태계·환경 설정·트러블슈팅은 어느 트랙에도 안 들어가는 별도 축이라 부록으로 뺐다."
date        : 2026-06-16 23:00:00 +0900
updated     : 2026-07-05 12:00:00 +0900
...
이 블로그에는 neovim·lazyvim 디렉토리에 걸쳐 50여 편의 Neovim 글이 있다. 처음 들어온 사람이 길을 잃지 않도록 순서를 묶되, **하나의 사다리로 세우지 않았다.** Neovim에는 결이 다른 두 질문이 있기 때문이다 — *어떻게 잘 쓰나*(사용자)와 *어떻게 만드나*(플러그인 개발). 이 둘을 한 줄로 이으면 "Lua 메타테이블을 알아야 LazyVim을 쓴다" 같은 부자연이 생긴다. 실제로는 그렇지 않다.

그래서 **공통 입문**에서 갈라지는 **두 트랙**으로 나눴다.

- **사용자 트랙 — 어떻게 잘 쓰나.** 편집 기본기에서 LazyVim 구조 이해·커스터마이징까지. 플러그인을 평생 안 만들어도 여기서 완결된다.
- **개발자 트랙 — 어떻게 만드나.** Lua 언어에서 런타임 API·플러그인 구조·발행·테스트까지. 플러그인을 짜려는 사람만 들어오면 된다.

**두 트랙은 초급→고급의 위아래가 아니라 목적이 다른 평행 갈래다.** 사용자 트랙을 다 타도 개발자 트랙은 선택이고, 그 반대도 성립한다. 그리고 어느 트랙에도 안 들어가는 결이 다른 축 — 옆 도구와의 비교·생태계, 프로젝트·언어별 환경 설정, 트러블슈팅 — 은 아래 **부록**으로 분리했다.

## 한눈에 보기

공통 입문에서 두 트랙으로 갈라진다. 트랙은 서로 선행조건이 아니다. 부록은 트랙 밖의 별도 트랙이다.

| 구역 | 답하는 질문 | 성격 |
|---|---|---|
| 입문 | Vim·Neovim·배포판이 뭐가 다른가 | 공통 출발점 |
| **사용자 트랙 · 편집 기본기** | 모션·레지스터로 빠르게 편집 | 척추 A |
| **사용자 트랙 · 설정·구조 이해** | LazyVim을 알고 고치기 | 척추 A |
| **개발자 트랙 · 언어** | 플러그인을 짤 Lua·Vimscript | 척추 B |
| **개발자 트랙 · 런타임 API** | 기능을 만드는 `vim.*` 메커니즘 | 척추 B |
| **개발자 트랙 · 구조·발행** | 배치·문서·등록 | 척추 B |
| **개발자 트랙 · 테스트·실전** | 검증하고 실제로 올리기 | 척추 B |
...
Vim/Neovim/배포판이 헷갈린다면, 어느 트랙을 타든 먼저 셋의 관계부터 잡자.
...
| [Vim & Neovim 작동 원리 정리](/posts/neovim/2025-10-04-vim-core-engine/) | 모드·버퍼·윈도우·탭, 명령 파이프라인 — 두 트랙 모두의 전제 |

---

# 사용자 트랙 — 어떻게 잘 쓰나

플러그인을 만들지 않아도 여기서 완결된다. 편집 기본기를 손에 익히고, 그 위에 LazyVim이 어떻게 얹혀 있는지를 알고 고치는 데까지.

## 편집 기본기

Vim의 진짜 심장은 모션·오퍼레이터·텍스트오브젝트로 편집을 "문장처럼" 조합하는 것이다. LazyVim을 쓰더라도 이 층은 그대로 깔려 있다.
...
LazyVim을 "그냥 쓰는" 단계에서 "어떻게 동작하는지 알고 고치는" 단계로. 이걸 모르면 extras override·spec merge에서 매번 막힌다. 여기서 설정을 손보는 데 필요한 Lua는 **테이블·`require` 수준**이면 충분하다 — 그 이상(메타테이블·에러 처리)은 개발자 트랙의 몫이니, 가볍게 [Lua 종합 가이드](/posts/neovim/2026-06-15-lua-syntax-guide/)로 문법만 훑고 오면 된다.
...
여기서 "Neovim 사용자"에서 "생태계 기여자"로 넘어간다. Lua 언어 → 런타임 API → 플러그인 구조·발행 → 테스트·실전 순으로, 빈 디렉토리에서 awesome-neovim 등록까지 올리는 실제 흐름이다. **사용자 트랙을 끝내야 들어올 수 있는 건 아니다** — 편집 기본기와 입문만 되어 있으면 바로 이 트랙으로 와도 된다.

## 언어 — Lua와 Vimscript

Neovim 플러그인을 짜려면 Lua는 피할 수 없다. 시리즈 4부작 + 성능 1편으로 구성했다. [Lua 종합 가이드](/posts/neovim/2026-06-15-lua-syntax-guide/)가 hub고, 거기서 심화 3편으로 분기한다. (Vimscript 갈래를 택할 거라면 아래 두 글을 함께.)
...
어느 트랙도 아닌 **별도 트랙**이다. 옆 도구와의 비교 — "이 도구가 대체 뭐랑 다른 거지"를 짚어 두면 선택이 명확해진다. 겹쳐 보이는 도구의 **경계선**을 긋는 글들.
...
글로벌 설정은 사용자 트랙에서 다뤘고, 여기는 "특정 프로젝트·언어에 맞춰 별도로 손보는" 축이다. 해당 상황이 생겼을 때 찾아 들어오면 된다.
...
두 트랙 흐름과 분리해서 모았다. 같은 에러를 만났을 때 검색해서 들어오는 용도.
...
- **Vim/Neovim/배포판이 헷갈린다면** 입문 세 글로 계층부터 — 두 트랙 공통.
- **잘 쓰고 싶다면** 사용자 트랙 — 편집 기본기 → LazyVim 구조 이해. 여기서 완결된다.
- **LazyVim만 쓰고 있었다면** 사용자 트랙의 "설정·구조 이해"로 바로.
- **플러그인을 만들고 싶다면** 개발자 트랙 — Lua → 런타임 API → 구조·발행 → 테스트·실전. 사용자 트랙을 다 안 거쳐도 된다.
