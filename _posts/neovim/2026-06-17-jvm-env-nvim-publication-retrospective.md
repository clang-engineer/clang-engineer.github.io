...
직접 만든 첫 OSS Neovim 플러그인 [jvm-env.nvim](https://github.com/clang-engineer/jvm-env.nvim)을 발행하고 보강 릴리즈(v0.1.1)까지 돌린 사이클을 정리한다. 절차 가이드는 [awesome-neovim에 내 플러그인 PR 보내기](./2026-06-12-awesome-neovim-pr-walkthrough.md), [노출 채널 비교](./2026-06-12-neovim-plugin-distribution.md)에 이미 있으니, 여기는 "보강 사이클에서 무엇을 배웠는가"가 중심이다.
...
절차·함정(BSD sed, PR 제목 백틱, 금지어, typos CI 등)은 [awesome-neovim PR 보내기](./2026-06-12-awesome-neovim-pr-walkthrough.md)에 정리해둔 게 있어 그대로 따라갔다. Programming Languages Support 섹션의 `nvim-java/nvim-java` 바로 아래에 한 줄 추가, [PR #2365](https://github.com/rockerBOO/awesome-neovim/pull/2365)로 제출.
