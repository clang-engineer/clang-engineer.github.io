...
레지스터는 용도별로 몇 갈래로 나뉩니다. 아무것도 지정하지 않으면 **unnamed 레지스터**(`"`)가 쓰이고, 여러 조각을 따로 담아두고 싶으면 `"a`~`"z` 같은 **named 레지스터**에 이름 붙여 보관합니다. 삭제·변경으로 unnamed가 덮이더라도 마지막 **yank**만 살려두는 건 `"0`이고, 반대로 삭제 내용이 어떤 레지스터도 건드리지 않게 버리려면 **black hole** `"_`를 씁니다. 시스템 클립보드와 주고받을 때는 `"+`(또는 X11 selection `"*`)를 사용합니다.

아래는 각 레지스터의 상세 정리입니다.
