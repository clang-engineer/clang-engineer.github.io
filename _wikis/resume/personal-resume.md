...
- database 장애 또는 네트워크 문제등으로 인한 업무 비효율을 개선하기 위해 개발 환경과 운영 환경에서의 profile을 구분하여 개발 환경에서는 h2 local file db 또는 실제 운영 환경과 동일한 docker instance를 사용할 수 있도록 환경을 구성하였습니다. 서버 배포 전에는 실제 운영환경과 동일한 database 환경에서 테스트를 진행하도록 하여 특정 db환경에서의 에러를 방지하였습니다.
