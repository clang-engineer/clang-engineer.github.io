...
- 서버 재배포 후에 이전 정적 자원들이 브라우저에 의해 캐싱되는 것을 방지하기 위해 webpack 빌드시 파일명에 해시값을 추가하여 캐싱을 방지하였습니다.
...
- 분산 환경에서는 hibernate 2차 캐시와 스프링 캐시를 redis를 사용하여 구성하였습니다. 이를 통해 서버간 데이터 공유를 통해 성능을 향상시켰습니다시
...
- frontend는 dev환경에서 빌드할 경우 source map을 사용하여 디버깅을 용이하게 하였습니다. logging 미들웨어를  추가하여  개발환경에서 store 의 동작을 구체적으로 확인할 수 있도록 하였습니다.
...
- 양방향 암호화를 적용하는 방식에 있어서 어플리케이션, 데이터베이스 계층이 검토 되었습니다. 이중 어플리케이션 레벨에서 암호화하는 경우 데이터베이스 레벨에서 암호화하는 경우보다 보안 강도가 높고, 유연한 관리와 테스트가 가능하다는 장점이 있기에 암호화 계층으로 선택하였습니다.
- 양방향 암호화를 위한 알고리즘으로 AES, 3DES등의 방식이 검토되었습니다. 이중 별도의 키를 사용하여 DES보다 보안 강도가 높고 속도가 빠른 AES를 사용하여 고과정보 암호화하였습니다.
...
# 캐시전략
READ_ONLY: 이 캐시 전략은 엔티티가 변경되지 않는다고 가정하고, 한 번 로드된 엔티티는 캐시에 저장된 후 다시 로드하지 않습니다. 따라서 데이터베이스에 대한 추가 조회가 발생하지 않습니다. 이 캐시 전략은 읽기 전용 데이터나 거의 변경되지 않는 데이터에 적합합니다.

NONSTRICT_READ_WRITE: 이 캐시 전략은 엔티티를 캐시에 저장하고, 변경 시 즉시 캐시를 업데이트합니다. 그러나 데이터베이스와의 일관성을 보장하기 위해 업데이트는 비동기적으로 수행됩니다. 이 캐시 전략은 변경되는 가능성이 있는 데이터에 적합합니다. 즉, 약간의 데이터 불일치가 허용되는 경우에 사용됩니다.

READ_WRITE: 이 캐시 전략은 엔티티를 캐시에 저장하고, 변경이 발생할 때 캐시를 즉시 업데이트하고 데이터베이스도 즉시 업데이트합니다. 이는 가장 엄격한 캐시 전략으로, 데이터베이스와 캐시가 항상 일관된 상태를 유지합니다. 이 캐시 전략은 변경 빈도가 높은 데이터에 적합합니다.

TRANSACTIONAL: 이 캐시 전략은 트랜잭션 범위 내에서 캐시된 데이터를 유지합니다. 즉, 트랜잭션이 커밋될 때까지 캐시된 데이터를 유지하고, 트랜잭션이 롤백되면 캐시가 롤백됩니다. 이 캐시 전략은 트랜잭션 내에서 일시적인 데이터를 캐시하고 싶을 때 사용됩니다.