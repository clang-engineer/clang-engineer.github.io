...
# 1. 개발 환경 구성
여러 개발자들이 원활하게 협업할 수 있도록 아래와 같은 개발 환경을 구성하였습니다.

## 1.1 프레임 워크
- backend는 spring boot를 사용하여 개발하였고 생산성 향상을 위해 java대신 kotlin을 사용하였습니다.
- frontend는 react를 사용하여 개발하였고 생산성 향상을 위해 javascript대신 typescript를 사용하였습니다.
...
- 빌드시에는 코드 압축, 난독화, 캐싱등의 최적화 설정을 추가하여 성능을 향상시켰습니다. (frontend: webpack.prod.js)
- 빌드시에는 코드를 배포하기 전에 코드 스타일을 점검하였습니다. (backend: ktlint, sonarqube, frontend: eslint, sonarqube)

## 1.3 테스트 도구
- backend는 junit을 사용하여 테스트 코드를 작성하고 jacoco를 사용하여 코드 커버리지를 검사하였습니다. sonarqube에 jacoco 리포트를 연동하여 코드 커버리지, 코드 품질을 개발자들이 쉽게 확인할 수 있도록 하였습니다.
- frontend는 jest를 사용하여 테스트 코드를 작성하였고 sonarqube에 jest 리포트를 연동하여 코드 커버리지, 코드 품질을 개발자들이 쉽게 확인할 수 있도록 하였습니다.
...
- 설계 과정에서 sticky, cluster, global, stateless 등의 세션관리 방식이 검토되었습니다. 이중 서버 증설 관점에서 sticky 세션은 한 서버에 과부하가 걸릴 수 있다는 단점이, cluster 세션은 모든 서버가 동일한 세션 객체를 가져야 하므로 많은 메모리를 소모하게되고 서버 개수가 증가함에 따라 동기화 과정에서 성능 저하가 크다는 단점이 있기 때문에 후보에서 제외하였습니다.
 ...
 개발하고 있는 솔루션의 특성을 고려했을 때 차후 사용자가 늘어남에 따라 확장은 필수적이지만, 별도의 세션 통제와 같은 global 세션방식의 이점이 불필요하였고, 고객사마다 특정 사설망에서 솔루션을 운영할 수 있어야했기 때문에 별도의 서드파티 운영 오버헤드없이 쉽게 부하분산할 수 있는 token 인증 방식을 선택하였습니다. 
...
- prettier를 통해서 eslint를 통해서 
- checkstyle을 통해서 ktlint를 통해서
- git template 을 통해 
