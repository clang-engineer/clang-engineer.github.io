...
- hibernate 1차 캐시(영속성 매니저)에 더해 2차 캐시를 사용하여 성능을 향상시켰습니다. 엔티티별로 성격에 때라 READ_ONLY, NONSTRICT_READ_WRITE, READ_WRITE, TRANSACTIONAL등의 캐시 전략을 사용하여 갱신 과정에서 발생하는 오버헤드를 줄였습니다. 분산 환경에서는 redis를 사용하여 2차 캐시를 구성하였습니다.
...
- 한 개체를 여러 개체에 나눠 저장하는 경우 Embeddable 사용하여 복잡도를 줄이도록 하였습니다. 이 과정에서 이를 사욜하여 한 개체 안의 속성을 별도로 구분하고 물리 테이블도 이에 따라 분리하였습니다.
