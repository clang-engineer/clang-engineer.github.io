...
- 단방향 암,복호화가 불필요한 패스워드와 키 암호화에 사용하였고, 양방향 암호화는 복호화가 필요한 고과정보에 사용하였습니다. 해싱은 고과정보들에 대한 최종 무결성을 보장하기 위해 사용하였습니다.
...
- 양방향 암호화를 적용하는 계층 선택에 있어서 어플리케이션, 데이터베이스 계층이 검토 되었습니다. 이중 어플리케이션 레벨에서 암호화하는 경우 데이터베이스 레벨에서 암호화하는 경우보다 보안 강도가 높고, 데이터베이스 레벨에서 암호화하는 경우보다 유연한 관리와 테스트가 가능하다는 장점이 있기에 어플리케이션 레벨에서 암호화를 적용하였습니다레
- 양방향 암호화를 위한 알고리즘으로 AES, 3DES등의 방식이 검토되었습니다. 이중 별도의 키를 사용하여 보안 강도가 높고 속도가 높은 AES를 사용하여 고과정보 암호화하였습니다.
