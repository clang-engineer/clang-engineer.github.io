...
- json server를 사용한 api서버를 구축하여 실제 api개발 전에 도메인 모델을 미리 구축하고 프론트엔드 개발을 독립적으로 진행할 수 있도록 하였습니다.
...
- eslint를 사용하여 코드 품질을 검사하였습니다.
- prettier를 사용하여 코드 스타일을 통일하였습니다.
- husky와 lint-staged를 사용하여 커밋 전에 prettier로 코드 포맷팅을 자동화하였습니다. (git pre-commit hook)


# 5. 성능 향상 정책
- WebMvcConfigurer 설정을 오버라이딩하여 정적 자원에 대한 최적화를 하였습니다. 정적 자원에 대한 응답시 Cache-Control 헤더 설정을 통해 브라우저가 캐싱 자원을 사용할 수 있도록 하였습니다. 또한, 서버 재배포 후에 변경된 자원들에 대한 브라우저 캐싱을 방지하기 위해 파일명에 해시값을 추가하여 캐싱을 방지하였습니다.
