...
이 문서는 서로 다른 추상화 수준의 개념이 **Application 원격 통신 흐름에서 어떻게 연결되는지**를 정리한다.
...
RPC를 구현한 구체 Framework
→ gRPC
```

따라서 IPC · RPC · gRPC를 같은 종류의 Protocol 목록으로 보지 않는다. Socket API, FD, Event Loop, Netty처럼 실제 실행 계층으로 더 내려가는 내용은 `_보충학습`에서 이어서 다룬다.
